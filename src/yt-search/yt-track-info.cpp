...
mime_type_t::mime_type_t(std::string str) {
    ...
    if (l_type_index) {
        ...
        if (l_format_index)
            this->format = str.substr(l_type_index + 1, l_format_index - (l_type_index + 1));
    }
    size_t l_codecs_index = str.find('"');
    if (l_codecs_index)
        this->codecs = str.substr(l_codecs_index + 1, str.length() - l_codecs_index - 2);
}

mime_type_t::~mime_type_t() = default;

int audio_info_t::itag() const { return raw.is_null() ? 0 : raw.value<int>("itag", 0); }

std::string audio_info_t::url() const { return raw.is_null() ? "" : raw.value<std::string>("url", ""); }

mime_type_t audio_info_t::mime_type() const { return mime_type_t(raw.is_null() ? "" : raw.value<std::string>("mimeType", "")); }

size_t audio_info_t::bitrate() const { return raw.is_null() ? 0 : raw.value<size_t>("bitrate", 0); }
...
std::pair<size_t, size_t> audio_info_t::init_range() const { return get_ranges(raw, "initRange"); }

std::pair<size_t, size_t> audio_info_t::index_range() const { return get_ranges(raw, "indexRange"); }
...
std::string audio_info_t::quality() const { return raw.is_null() ? "" : raw.value<std::string>("quality", ""); }

std::string audio_info_t::projection_type() const { return raw.is_null() ? "" : raw.value<std::string>("projectionType", ""); }

size_t audio_info_t::average_bitrate() const { return raw.is_null() ? 0 : raw.value<uint64_t>("averageBitrate", 0); }

std::string audio_info_t::audio_quality() const { return raw.is_null() ? "" : raw.value<std::string>("audioQuality", ""); }
